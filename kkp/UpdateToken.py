...
geturl = r'https://api.github.com/repos/' + \
    gh_repo+r'/actions/secrets/public-key'
# puturl=r'https://api.github.com/repos/'+gh_repo+r'/actions/secrets/MS_TOKEN'
key_id = 'kXjGAo3VmfdwyYGt'
...
def setsecret(encrypted_value, key_id, puturl, appnum):
    ...
    for retry_ in range(4):
        ...
        if putstatus.status_code < 300:
            ...
            if retry_ == 3:
                print(r'账号/应用 '+str(appnum) +
                      ' 的微软密钥上传失败，请检查secret里 GH_TOKEN 格式与设置是否正确')
    return putstatus
...
for a in range(1, int(app_num)+1):
    ...
    if a == 1:
        ...
        puturl = r'https://api.github.com/repos/' + \
            gh_repo+r'/actions/secrets/MS_TOKEN_'+str(a)
    encrypted_value = createsecret(getpublickey(
        Auth, geturl), getmstoken(ms_token, a))
